...
import com.beagleapps.android.trimettracker.PreferencesHelper.DisplayChoice;
import com.beagleapps.android.trimettracker.PreferencesHelper.SortMethods;
...
public class HistoryHelper {
	...
	static long addFavoriteToHistory(SQLiteDatabase database, Favorite favorite){
		...
		values.put(DBHelper.COL_HT_STOPID, favorite.stopID);
		values.put(DBHelper.COL_HT_LAST_VISITED, HistoryHelper.getCurrentTime());
		values.put(DBHelper.COL_HT_VISITS, 1);
		values.put(DBHelper.COL_DESCRIPTION, favorite.description);
		values.put(DBHelper.COL_DIRECTION, favorite.direction);
		values.put(DBHelper.COL_ROUTES, favorite.routes);
